...
intermediate_sampling_grid_R = 1e4         # wavelength array to rebin to, higher resolution means higher oversampling
...
# MAIN CACHE BUILDER
# ==========================================
def create_cache_parallel(mol_name, file_list, Tg, Ntot, output_dir, r_miri, cores_setting, intermediate_sampling_grid_R):
    """Dispatches cache building across multiple CPU cores."""
    n_T, n_N = len(Tg), len(Ntot)
    total_models = n_T * n_N
    intermediate_wave = c / rs.generate_grid(lambda_0=wavelength_range[0], lambda_n=wavelength_range[1], R=intermediate_sampling_grid_R) * 1e-3
...
if __name__ == '__main__':
    ...
    create_cache_parallel(
        ...
        intermediate_sampling_grid_R = intermediate_sampling_grid_R
    )